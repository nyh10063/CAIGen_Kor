...
이 저장소는 논문 「언어 모델과 규칙을 활용한 한국어 문법 항목 자동 주석 연구」의 전체 자동 주석 시스템을 공개하는 저장소가 아니라, 연구 과정에서 문법 항목 스팬을 수동 검토·산출하기 위해 사용한 CAIGen_Kor 인터페이스와 전처리 도구를 공개하는 저장소입니다. 논문 실험에 사용한 전체 문법항목 사전, 규칙 세트, gold data, 학습 데이터, 말뭉치 원문은 저작권 및 연구 자산 보호를 위해 포함하지 않습니다. 이 저장소에는 실행 확인을 위한 최소 샘플 데이터가 포함되어 있습니다.
...
This repository is based on CAIGen by Yusuke Ide et al.
Original repository: https://github.com/Yusuke196/CAIGen
